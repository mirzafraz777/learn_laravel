    ...
    <div class="mx-4">
        <x-card class="p-10 max-w-lg mx-auto mt-24">
            <header class="text-center">
                ...
                <p class="mb-4">Edit/Update A GIG <b>{{$listing->title}} </b>.</p>
            </header>
            ...
            <form method ="POST" action="/listing/{{$listing->id}}" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <img src="/storage/{{$listing->logo}}" alt="Company Logo" height="80" width="80">
                    @error('logo')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea class="border border-gray-200 rounded p-2 w-full" name="description"
                        rows="10" placeholder="Include tasks, requirements, salary, etc">{{$listing->description}}
                    </textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
